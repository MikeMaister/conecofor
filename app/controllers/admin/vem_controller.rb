class Admin::VemController < ApplicationController
  ...
  def show
    ...
    @vem_data_1200 = Cops.find_by_sql ["select campagne_id,plot_id,specie_id,codice_strato,in_out,priest,sum((select value from copertura_specifica where copertura_specifica_id = id)) as tot_copertura, substrate_type_id,certainty_species_determination_id from cops where approved = true AND deleted = false AND campagne_id IN (SELECT id FROM campagne WHERE deleted = false AND anno = ?) group by plot_id,specie_id,codice_strato,in_out,priest",anno]
    @vem_data_400_in = Cops.find_by_sql ["select campagne_id,plot_id,specie_id,codice_strato,in_out,priest,sum((select value from copertura_specifica where copertura_specifica_id = id)) as tot_copertura, substrate_type_id,certainty_species_determination_id from cops where in_out = 1 and subplot in(4,6,7,9) and approved = true AND deleted = false AND campagne_id IN (SELECT id FROM campagne WHERE deleted = false AND anno = ?) group by plot_id,specie_id,codice_strato,priest",anno]
    @vem_data_400_out = Cops.find_by_sql ["select campagne_id,plot_id,specie_id,codice_strato,in_out,priest,sum((select value from copertura_specifica where copertura_specifica_id = id)) as tot_copertura, substrate_type_id,certainty_species_determination_id from cops where in_out = 2 and subplot in (3,5,7,9) and approved = true AND deleted = false AND campagne_id IN (SELECT id FROM campagne WHERE deleted = false AND anno = ?) group by plot_id,specie_id,codice_strato,priest",anno]
