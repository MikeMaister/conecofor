module Legn_checks
  ...
  def habitual_species(record)
    ...
    dati_approvati = Legnose.find(:all,:conditions => ["approved = true and temp = false and deleted = false and plot_id",record.plot_id])
    unless dati_approvati.blank?
      #carico il file
      file = ImportFile.find(session[:file_id])
      #estrapolo tutte le specie abituali del plot
      habitual = Legnose.find_by_sql ["select distinct specie_id as specie from legnose where plot_id = ? and temp = false and approved = true and deleted = false",record.plot_id]
      trovato = false
